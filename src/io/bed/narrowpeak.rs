...
//! narrowPeak format reading and writing.
//!
//! # Example
//!
//! ```
//! use bio::io::narrow_peak;
//!
//! let example = b"1\t5\t5000\tpeak1\t0.5\t.\t182\t2.0945\t-1\t50";
//! let mut reader = narrow_peak::Reader::new(&example[..]);
//! let mut writer = narrow_peak::Writer::new(vec![]);
...
/// A narrowPeak record as defined by UCSC
...
/// Add new methods specific for a narrowPeak Record
impl Record {
    /// Create a new Record
    pub fn new() -> Self {
        Self::default()
    ...
    /// Signal Value: Measurement of overall (usually, average) enrichment for the region.
    pub fn signal_value(&self) -> Option<&str> {
        self.aux(6)
    }

    /// pValue: Measurement of statistical significance (-log10). Use -1 if no pValue is assigned.
    pub fn pvalue(&self) -> Option<&str> {
        self.aux(7)
    }

    /// qValue: Measurement of statistical significance using false discovery rate (-log10). Use -1 if no qValue is assigned.
    pub fn qvalue(&self) -> Option<&str> {
        self.aux(8)
    }

    /// Peak: Point-source called for this peak; 0-based offset from Start. Use -1 if no point-source called.
    ...
    /// Set Signal Value: Measurement of overall (usually, average) enrichment for the region.
    /// It should be between 0-1000, but no checks will be performed.
    pub fn set_signal_value(&mut self, signal_value: u64) {
        self.set_aux(6, signal_value.to_string().as_str())
    }

    /// Set pValue: Measurement of statistical significance (-log10). Use -1 if no pValue is assigned.
    pub fn set_pvalue(&mut self, p_value: i64) {
        self.set_aux(7, p_value.to_string().as_str())
    }

    /// Set qValue: Measurement of statistical significance using false discovery rate (-log10). Use -1 if no qValue is assigned.
    pub fn set_qvalue(&mut self, q_value: i64) {
        self.set_aux(8, q_value.to_string().as_str())
    }

    /// Set Peak: Point-source called for this peak; 0-based offset from Start. Use -1 if no point-source called.
    pub fn set_peak(&mut self, peak: i64) {
        self.set_aux(9, peak.to_string().as_str())
    }
}
